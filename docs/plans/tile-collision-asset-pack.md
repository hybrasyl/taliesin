...
1. **The dat-files.md `& 0xF0` mask claim.** [dat-files.md:190](./dat-files.md#L190) states the client masks every SOTP byte with `& 0xF0` after read. The empirical byte distribution (`0x00` / `0x0f` / `0x80` / `0x8f`) can't square with that as the only step: `0x0f & 0xF0 = 0x00` would mark every retail wall passable, and tile 99 (raw byte `0x0f`) is empirically impassable. The most likely explanations: (a) the mask is actually `& 0x0F` and dat-files.md got the direction wrong, or (b) the masked value is used for a non-collision purpose and collision is read pre-mask. Re-reading `FUN_005CF3B0` would settle it. Doesn't block this format — the override merge uses the empirical low-nibble rule. Note: do not edit dat-files.md or anything in the protocol/reference doc tree as part of this work; if the disassembly is ever done, the finding goes back into this scoping doc.
