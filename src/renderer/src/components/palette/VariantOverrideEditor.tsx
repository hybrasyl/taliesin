...
import { Box, Button, IconButton, Stack, TextField, Typography, Slider, Tooltip } from '@mui/material'
  ...
  return (
    <Box sx={{ p: 1.5 }}>
      <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
        <Typography variant="body2" sx={{ fontWeight: 'bold' }}>Variants</Typography>
        ...
        {customizing ? (
          <Button size="small" variant="outlined" startIcon={<RestartAltIcon />} onClick={resetToDefaults}>
            Reset to Defaults
          </Button>
        ) : (
          ...
          {variants!.map((v, i) => (
            ...
            >
              <TextField
                ...
                size="small"
                value={v.id}
                onChange={e => updateAt(i, { id: e.target.value.toLowerCase().replace(/\s+/g, '_') })}
                sx={{ width: 120 }}
              />
              <TextField
                label="Label"
              ...
              <Box sx={{ flex: 1, display: 'flex', gap: 2, minWidth: 360 }}>
                <Box sx={{ flex: 1 }}>
                  <Typography variant="caption" color="text.secondary">Dark: {v.darkFactor.toFixed(2)}</Typography>
                  <Slider size="small" min={0} max={1} step={0.05} value={v.darkFactor}
                    onChange={(_, val) => updateAt(i, { darkFactor: val as number })} />
                </Box>
                <Box sx={{ flex: 1 }}>
                  <Typography variant="caption" color="text.secondary">Light: {v.lightFactor.toFixed(2)}</Typography>
                  <Slider size="small" min={0} max={1} step={0.05} value={v.lightFactor}
              ...
              <Tooltip title="Delete variant">
                <IconButton size="small" onClick={() => removeAt(i)}><DeleteIcon fontSize="small" /></IconButton>
              </Tooltip>
            </Box>
          ))}
          <Box sx={{ pt: 1 }}>
            <Button size="small" variant="outlined" startIcon={<AddIcon />} onClick={addVariant}>Add Variant</Button>
          </Box>
        </>
      )}
    </Box>
