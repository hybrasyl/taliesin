        ...
        <Box sx={{ pt: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          {isBoard && (
            <TextField
              ...
              helperText="Unique key identifying this board"
            />
          )}
          <TextField
            label="Name"
          ...
          <TextField
            ...
            helperText="Text displayed when a player reads the sign"
          />
          <ScriptAutocomplete
            label="Script"
            value={script}
            onChange={setScript}
            helperText="Optional — Lua script executed when a player interacts"
          />
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5 }}>
            <TextField
              label="On Entry Effect"
              size="small"
              type="number"
              value={onEntry}
              onChange={(e) => setOnEntry(e.target.value)}
              helperText="Animation ID on player entry"
            />
            <TextField
              label="Entry Speed"
              size="small"
              type="number"
              value={onEntrySpeed}
              onChange={(e) => setOnEntrySpeed(e.target.value)}
      ...
      <DialogContent>
        <Box sx={{ pt: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <ScriptAutocomplete
            ...
            value={script}
            onChange={setScript}
            helperText="Lua script executed when a player steps on the tile"
          />
        </Box>
      </DialogContent>
      <DialogActions>
...
export function MusicIdField({
  ...
  const playTooltip = !clientPath
    ...
    : value == null
      ? 'Set a Music Id to preview'
      : !fileExists
        ? `${value}.mus not found in client/music`
        : playing
          ? 'Stop preview'
          : 'Preview music'
...
function MapFieldsTab({
  ...
  const setSg = (patch: Partial<MapSpawnGroup>) =>
    onChange((prev) => ({
      ...prev,
      spawnGroup: { ...(prev.spawnGroup ?? { name: '', baseLevel: 1, spawns: [] }), ...patch }
    }))
  ...
  const addSpawnGroup = () =>
    onChange((prev) => ({
      ...
      spawnGroup: {
        name: sgNameHint,
        baseLevel: 1,
        spawns: [{ import: '', flags: ['Active'] as MapSpawnFlag[] }]
      }
    }))

  const removeSpawnGroup = () => onChange((prev) => ({ ...prev, spawnGroup: undefined }))
  ...
  const addSpawn = () =>
    onChange((prev) => ({
      ...
      spawnGroup: {
        ...prev.spawnGroup!,
        spawns: [...prev.spawnGroup!.spawns, { import: '', flags: ['Active'] as MapSpawnFlag[] }]
      }
    }))
  ...
  const updateSpawn = (i: number, s: MapSpawn) =>
    onChange((prev) => ({
      ...
      spawnGroup: {
        ...prev.spawnGroup!,
        spawns: prev.spawnGroup!.spawns.map((x, idx) => (idx === i ? s : x))
      }
    }))
  ...
  const removeSpawn = (i: number) =>
    onChange((prev) => ({
      ...
      spawnGroup: {
        ...prev.spawnGroup!,
        spawns: prev.spawnGroup!.spawns.filter((_, idx) => idx !== i)
      }
    }))

  const toggleSpawnFlag = (i: number, flag: MapSpawnFlag) => {
    const spawn = data.spawnGroup!.spawns[i]
    const next = spawn.flags.includes(flag)
      ? spawn.flags.filter((f) => f !== flag)
      : [...spawn.flags, flag]
      ...
      <Paper variant="outlined" sx={{ mb: 1, p: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1.5, mb: 1.5, alignItems: 'center', flexWrap: 'wrap' }}>
          <Tooltip
            title={
              !mapDirectory
                ? 'Configure a map directory in Settings to use the picker'
                : !data.id
                  ? 'Set a Map ID first'
          ...
          <FormControlLabel
            control={
              <Switch
                size="small"
                checked={data.allowCasting}
                onChange={(e) => set('allowCasting', e.target.checked)}
              />
            }
            label="Casting"
          />
          <FormControlLabel
            control={
              <Switch
                size="small"
                checked={data.dynamicLighting}
                onChange={(e) => set('dynamicLighting', e.target.checked)}
              />
            }
          ...
          {!data.spawnGroup ? (
            ...
            <>
              ...
              {data.spawnGroup.spawns.map((spawn, i) => (
                <Paper key={i} variant="outlined" sx={{ p: 1, mb: 1 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.75 }}>
                    <Autocomplete
                      freeSolo
                      options={spawnGroupNames}
                      value={spawn.import}
                      size="small"
                      onInputChange={(_, v) => updateSpawn(i, { ...spawn, import: v })}
                      sx={{ flex: 1 }}
                      renderInput={(params) => (
                        <TextField {...params} label="Import" size="small" />
                      )}
                    />
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 0.5 }}>
                ...
                <Button size="small" color="error" onClick={removeSpawnGroup}>
                  Remove Group
                </Button>
              </Box>
            </>
...
function MapPlacementTab({
  ...
  const confirmNpc = (npc: MapNpc) => {
    ...
    if (ds.editIndex !== undefined) {
      onChange((prev) => ({
        ...prev,
        npcs: prev.npcs.map((n, i) => (i === ds.editIndex ? npc : n))
      }))
  ...
  const confirmWarp = (warp: MapWarp) => {
    const ds = dialogState
    if (!ds || ds.kind !== 'warp') return
    if (ds.editIndex !== undefined) {
      onChange((prev) => ({
        ...prev,
        warps: prev.warps.map((w, i) => (i === ds.editIndex ? warp : w))
      }))
  ...
  const confirmSign = (sign: MapSign) => {
    const ds = dialogState
    if (!ds || ds.kind !== 'sign') return
    if (ds.editIndex !== undefined) {
      onChange((prev) => ({
        ...prev,
        signs: prev.signs.map((s, i) => (i === ds.editIndex ? sign : s))
      }))
  ...
  const confirmReactor = (reactor: MapReactor) => {
    const ds = dialogState
    if (!ds || ds.kind !== 'reactor') return
    if (ds.editIndex !== undefined) {
      onChange((prev) => ({
        ...prev,
        reactors: prev.reactors.map((r, i) => (i === ds.editIndex ? reactor : r))
      }))
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', gap: 1 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
        ...
        <Tooltip title="Fit (reset zoom)">
          <IconButton size="small" onClick={() => setZoomIdx(1)}>
        ...
        <Tooltip
          title={
            showPassability
              ? 'Hide passability overlay'
              : 'Show passability overlay (requires DA client path)'
          }
        >
          <IconButton
            size="small"
            onClick={() => setShowPassability((v) => !v)}
            color={showPassability ? 'warning' : 'default'}
          >
          ...
          >
            ...
            <ItemsGroup
              ...
              items={data.npcs.map((n, i) => ({
                key: i,
                label: `(${n.x},${n.y}) ${n.name || '?'}`,
                selected: selected?.kind === 'npc' && selected.index === i,
                onSelect: () =>
                  setSelected((s) =>
                    s?.kind === 'npc' && s.index === i ? null : { kind: 'npc', index: i }
                  ),
                onEdit: () => openEdit('npc', i),
                onRemove: () => removeItem('npc', i)
              }))}
            />
            <Divider />
            <ItemsGroup
              ...
              items={data.signs.map((s, i) => ({
                key: i,
                label: `(${s.x},${s.y}) [${s.type}]${s.name ? ` ${s.name}` : ''}`,
                selected: selected?.kind === 'sign' && selected.index === i,
                onSelect: () =>
                  setSelected((s2) =>
                    s2?.kind === 'sign' && s2.index === i ? null : { kind: 'sign', index: i }
                  ),
                onEdit: () => openEdit('sign', i),
                onRemove: () => removeItem('sign', i)
              }))}
            />
            <Divider />
            <ItemsGroup
              ...
              items={data.reactors.map((r, i) => ({
                key: i,
                label: `(${r.x},${r.y})${r.displayName ? ` ${r.displayName}` : r.script ? ` [${r.script}]` : ''}`,
                selected: selected?.kind === 'reactor' && selected.index === i,
                onSelect: () =>
                  setSelected((s) =>
                    s?.kind === 'reactor' && s.index === i ? null : { kind: 'reactor', index: i }
                  ),
                onEdit: () => openEdit('reactor', i),
                onRemove: () => removeItem('reactor', i)
              }))}
            />
          </Box>
        </Box>
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <Box sx={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        {tab === 0 && (
          <MapFieldsTab data={data} spawnGroupNames={spawnGroupNames} onChange={handleChange} />
        )}
        {tab === 1 && (
          <MapPlacementTab
            data={data}
