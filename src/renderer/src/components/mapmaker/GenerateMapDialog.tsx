...
import {
  ...
  FormControl, Slider, IconButton, Tooltip, Divider,
} from '@mui/material'
import CasinoIcon from '@mui/icons-material/Casino'
import AutoAwesomeIcon from '@mui/icons-material/AutoAwesome'
import { MapFile } from '@eriscorp/dalib-ts'
import {
  generateTerrain, TERRAIN_DEFAULTS,
  type TerrainParams,
...
function wallFamilyLabel(f: { id: string; pairs: [number, number][]; totalFrequency: number }): string {
...
const GenerateMapDialog: React.FC<Props> = ({ open, onClose, onGenerate }) => {
  ...
  const handleGenerate = useCallback(() => {
    ...
    terrainSecondaryThreshold, terrainWallDensity, terrainWallThreshold,
    primaryFamilyIdx, secondaryFamilyIdx, wallFamilyIdx,
    onGenerate, onClose,
  ])
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent dividers>
        ...
        {/* ── Tile Families ──────────────────────────────────────────── */}
        <Typography variant="subtitle2" gutterBottom>Tile Families</Typography>

        <Box sx={{ display: 'flex', gap: 2, mb: 2, flexWrap: 'wrap' }}>
          <FormControl size="small" sx={{ minWidth: 260 }}>
            <InputLabel>Primary Ground</InputLabel>
            <Select label="Primary Ground" value={primaryFamilyIdx}
              onChange={e => setPrimaryFamilyIdx(Number(e.target.value))}>
              {topBgFamilies.map((f, i) => (
                <MenuItem key={f.id} value={i}>{familyLabel(f)}</MenuItem>
              ))}
            </Select>
          </FormControl>
          ...
          <FormControl size="small" sx={{ minWidth: 260 }}>
            ...
            <Select label="Secondary Ground" value={secondaryFamilyIdx}
          ...
          <FormControl size="small" sx={{ minWidth: 260 }}>
            ...
            <Select label="Wall Style (optional)" value={wallFamilyIdx}
              onChange={e => setWallFamilyIdx(Number(e.target.value))}>
              <MenuItem value={-1}><em>None</em></MenuItem>
              {topWallFamilies.map((f, i) => (
                <MenuItem key={f.id} value={i}>{wallFamilyLabel(f)}</MenuItem>
              ))}
            </Select>
          </FormControl>
        </Box>
        ...
        {/* ── Noise Parameters ───────────────────────────────────────── */}
        <Typography variant="subtitle2" gutterBottom>Noise Parameters</Typography>
        ...
        <Slider
          value={terrainSecondaryThreshold} min={0.1} max={0.9} step={0.05}
        ...
        {wallFamilyIdx >= 0 && (
          <>
            ...
            <Slider
              value={terrainWallDensity} min={0} max={0.3} step={0.01}
            ...
            <Slider
              value={terrainWallThreshold} min={0.3} max={0.95} step={0.05}
