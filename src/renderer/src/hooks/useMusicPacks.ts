...
export function useMusicPacks(libraryDir: string | null) {
  ...
  const addTrack = useCallback(
    async (packId: string, sourceFile: string, musicId: number) => {
      const updated = packs.map((p) => {
  ...
  const removeTrack = useCallback(
    async (packId: string, sourceFile: string) => {
      const updated = packs.map((p) =>
        p.id !== packId
          ? p
          : {
  ...
  const reorderTracks = useCallback(
    async (packId: string, tracks: MusicPackTrack[]) => {
      const updated = packs.map((p) =>
