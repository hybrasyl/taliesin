...
describe('classifyEntry', () => {
  const cases: [string, ReturnType<typeof classifyEntry>][] = [
    // Sprites
    ['mob.epf', 'sprite'], ['cape.spf', 'sprite'], ['walk.mpf', 'sprite'],
    ['blast.efa', 'sprite'], ['banner.hpf', 'sprite'],
    // Palette
    ['mpt001.pal', 'palette'],
    // Text
    ['readme.txt', 'text'], ['palette.tbl', 'text'], ['debug.log', 'text'],
    ['credits.nfo', 'text'],
    // Audio
    ['theme.mp3', 'audio'], ['ambient.wav', 'audio'], ['ogg.ogg', 'audio'],
    ['1.mus', 'audio'],
    ...
    ['data.bin', 'hex'], ['unknown.lft', 'hex'], ['weird.fak', 'hex'],
  ]
...
describe('parseBikHeader', () => {
  ...
  it('parses a typical BIKi header', () => {
    const info = parseBikHeader(buildHeader({
      version: 'i', frameCount: 181, width: 640, height: 480,
      fpsDividend: 30000, fpsDivisor: 1000, audioTracks: 1,
    }))
...
describe('decodePcx', () => {
   ...
   */
  function buildPcx(width: number, height: number, pixels: number[], palette: [number, number, number][]): Uint8Array {
  ...
  it('decodes a 2×2 image with a known palette', () => {
    ...
    expect([out!.rgba[12], out!.rgba[13], out!.rgba[14], out!.rgba[15]]).toEqual([255, 255, 255, 255])
  })
